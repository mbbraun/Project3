...
cur.execute('DROP TABLE IF EXISTS PROJECT3_TWEETS')
...
for tweet in umich_tweets:
	t1 = (tweet['user']['id_str'], tweet['user']['screen_name'],tweet['user']['favourites_count'],tweet['user']['description'])
	cur.execute(statement1, t1)
...
for tweet in umich_tweets:
	for user in tweet['entities']['user_mentions']:
		...
		t2 = (my_var['id_str'],my_var['screen_name'],my_var['favourites_count'],my_var['description'])
		cur.execute(statement1,t2)
...
for r in result.fetchall():
	for p in r:
		descriptions_fav_users.append(p)
...
word = ""
for text in description_words:
	word = word + text
rword = word.lower()
tup = collections.Counter(rword).most_common()
#print (tup)
